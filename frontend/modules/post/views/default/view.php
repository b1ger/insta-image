    ...
    <div class="row">
        <div class="col col-lg-6">
            <?php if ($post->hasComments($post->id)) : ?>
                <h2>Comments: <?php echo Yii::$app->redis->get("comments:{$post->id}:post"); ?></h2>
                <hr>
                <?php foreach ($post->comments as $comment) : ?>
                    ...
                    <?php if (!Yii::$app->user->isGuest && ($comment->username == $currentUser->username)) : ?>
                        <a href="<?php echo Url::to(['/post/comment/edit', 'commentId' => $comment->id, 'postId' => $post->id]); ?>" class="btn btn-primary">Edit</a>
                        <a href="<?php echo Url::to(['/post/comment/delete', 'id' => $comment->id]); ?>" class="btn btn-danger">Delete</a>
                    <?php endif; ?>
                    <hr>
                <?php endforeach; ?>
            <?php endif; ?>
            ...
            <?php if (!Yii::$app->user->isGuest) : ?>
                <?php $form = ActiveForm::begin(); ?>
                    ...
                    <?php echo $form->field($commentForm, 'text')->textarea(['rows' => 4]); ?>
                    <?php echo Html::activeHiddenInput($commentForm, 'username', ['value' => Yii::$app->user->identity->username]); ?>
